              ...
              <div v-for="data in orderedSettings" :key="data.id" class="pl-2 pt-3" >
                <b-row style="text-transform: uppercase; color:#717171;"
                  ...
                  ><b-col class="d-flex flex-row-reverse mb-2"
                    ><b-button variant="outline-danger ml-2 p-1" v-b-popover.hover="'Delete'" v-b-modal.settingPortalInfoDelete @click="selectedItem = data">
                      <i class="simple-icon-trash"></i></b-button
                    ><b-button variant="outline-success ml-2 p-1" v-b-popover.hover.left="'Edit'" v-b-modal.settingPortalInfoEdit @click="selectedItem = data">
                ...
                <b-row class="m-1">
                  ...
                  <input cols="7" type="text" v-model="data.ftpHost" class="portal-info" style="width:max-content!important;text-decoration: underline;" disabled />
                </b-row>
                <b-row class="m-1">
                  <b-col cols="3" style="color:#717171;"> Password: </b-col>
                  <input cols="7" type="password" v-model="data.password" class="portal-info" style="width:min-content" disabled />
                </b-row>
              </div>
              <edit-publishing-modal :item="selectedItem" @fetchPublishings="loadPublishings" />
              <delete-publishing-modal :setting-id="selectedItem.id" @fetchPublishings="loadPublishings" />
            </b-card-text>
          </template>
        </b-card>
      </b-colxx>
  ...
  methods: {
    ...
    async getPublicationsData() {
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        this.$notify("Success", "Request made: Get publishing settings", "Result: Publishing settings fetched successfully.", {
          type: "success",
          permanent: false,
          duration: 5000,
        });
        ...
        this.$notify("Error", "Request made: Get publishing settings", "Result: Publishing settings could not be fetched. Respose sent:" + err.status ==
            400 || 401
            ? "Access denied!"
